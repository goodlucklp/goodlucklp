...
static void ReportEventPkgCallback(const EventPackage **pkgs, uint32_t count, uint32_t devIndex)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        std::cout << "device action " << __func__<<" Index: "<< i << " "<< "devIndex: "
                  << devIndex << " type: " << pkgs[i]->type << " code: "
                  << pkgs[i]->code << " value: "<< pkgs[i]->value << " "<< std::endl;
    }
}
...
void ReportHotPlugEventPkgCallback(const HotPlugEvent *msg)
{
    ...
    std::cout << "device hotplug action " << __func__ <<" status: "<< msg->status <<" devIndex: "
              << msg->devIndex << " devType: "<< msg->devType << " "<< std::endl;
}

HWTEST_F(HdiInputTest, ScanInputDevice, TestSize.Level1)
{
    DevDesc sta[MAX_DEVICES] = {0};
    printf("%s: [Input] RegisterCallbackAndReportData001 enter %d", __func__, __LINE__);
    int32_t ret;
...
HWTEST_F(HdiInputTest, OpenInputDev001, TestSize.Level1)
{
    ...
    INPUT_CHECK_NULL_POINTER(g_inputInterface->iInputManager, INPUT_NULL_PTR);
    printf("%s: [Input] OpenInputDev001 enter %d\n", __func__, __LINE__);
    int32_t ret = g_inputInterface->iInputManager->OpenInputDevice(TOUCH_INDEX+1);
    printf("%s: [Input] OpenInputDev001 enter %d\n", __func__, __LINE__);
    if (ret) {
...
HWTEST_F(HdiInputTest, CloseInputDevice001, TestSize.Level1)
{
    ...
    INPUT_CHECK_NULL_POINTER(g_inputInterface, INPUT_NULL_PTR);
    INPUT_CHECK_NULL_POINTER(g_inputInterface->iInputManager, INPUT_NULL_PTR);
    int32_t ret = g_inputInterface->iInputManager->CloseInputDevice(TOUCH_INDEX+1);
    printf("%s: [Input] CloseInputDev001 enter %d\n", __func__, __LINE__);
    if (ret) {
...
HWTEST_F(HdiInputTest, GetInputDevice001, TestSize.Level1)
{
    ...
    printf("GetInputDevice001 %s: chipInfo = %s, vendorName = %s, chipName = %s\n",
...
HWTEST_F(HdiInputTest, GetInputDeviceList001, TestSize.Level1)
{
    ...
    for (uint32_t i = 0; i < num; i++) {
        ...
        printf("%s: index = %u, devType = %u", __func__, (dev + i)->devIndex,
               (dev + i)->devType);
...
HWTEST_F(HdiInputTest, RegisterCallbackAndReportData001, TestSize.Level1)
{
    ...
    EXPECT_EQ(ret, INPUT_SUCCESS);
    printf("%s: wait 10s for testing, pls touch the panel now", __func__);
